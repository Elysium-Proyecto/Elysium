...
document.getElementById("add-to-agenda").addEventListener("click", function () {
  ...
  const agenda = JSON.parse(localStorage.getItem("agenda_pre")) || [];
  agenda.push({ titulo, fechaEntrega, entregado: false });
  localStorage.setItem("agenda_pre", JSON.stringify(agenda));

  // Redirigir a la agenda
  window.location.href = "agenda_pre.html";
});
...
function verTarea(index) {
  ...
  window.location.href = "./form/vista-previa.html";
}
